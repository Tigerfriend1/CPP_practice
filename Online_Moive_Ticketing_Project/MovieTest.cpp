...
int main() {
    ...
    std::vector<DiscountConditionPtr> conditions {condition1};

    //요구사항2 : 비율할인정책을 이용한 요금계산이 이뤄지게 하라.
    //DiscountPolicyPtr policy = std::make_shared<AmountDiscountPolicy>(Money{2'000L}, conditions);
    DiscountPolicyPtr policy = std::make_shared<PercentDiscountPolicy>(10'000L,30, conditions);
    Movie movie {"Doom"s, 120, Money(10'000L), policy};

    time_t t = create_movie_time(30, 12, 13, 1, 2022);
    std::shared_ptr<Screening> screening = std::make_shared<Screening>(movie, 1, *std::localtime(&t));
